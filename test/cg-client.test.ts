...
describe('CommonGroundClient', () => {
  it('authenticates without exposing the token in the URL', async () => {
    const fetchMock = vi.fn<typeof fetch>().mockResolvedValue(
      new Response(
        JSON.stringify({ status: 'OK', data: { userId: 'u', deviceId: 'd', tokenId: 't' } }),
        { status: 200, headers: { 'content-type': 'application/json' } },
      ),
    );
    const client = new CommonGroundClient('https://cg.example', 'cgb_secret', communityId, channelId, fetchMock);
    await expect(client.whoami()).resolves.toEqual({ userId: 'u', deviceId: 'd', tokenId: 't' });

    const [url, init] = fetchMock.mock.calls[0]!;
    expect(url).toBe('https://cg.example/api/v2/Bot/whoami');
